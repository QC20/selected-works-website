...
| Program    | Removed                                                                                   |
| ---------- | ----------------------------------------------------------------------------------------- |
| jspaint    | `cypress/`, packaging and lint config, `discord-activity/`, `lib/tracky-mouse/` (its script tags are commented out upstream), pdf.js source maps and its standalone viewer (`build/pdf.js`, `build/pdf.worker.js` and `web/cmaps/` are kept, which is all jspaint asks for) |
| calculator | `mcalculator/engine/` — the C++ sources the engine is built from; only `mcalculator/server/public/` is loaded |
